...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	private void init(Context context, AttributeSet attrs) {
		// getPullToRefreshScrollDirection(abstract)判断滑动的方向，一般在自定义的继承View中实现
		switch (getPullToRefreshScrollDirection()) {
			case HORIZONTAL:
				// 设置Linearlayout的orientation，并且requestLayout
		...
		// 这里添加实体View，比如ListView等
		// createRefreshableView是个抽象函数，它具体实现在继承类中
		mRefreshableView = createRefreshableView(context, attrs);
		// 添加到一个FrameLayout中，再添加到LinearLayout容器中
		addRefreshableView(context, mRefreshableView);

		// We need to create now layouts now
		// 创建LoadingLayout,LoadingLayout是个Framelayout
	...
	public static enum AnimationStyle {
		...
		// 根据LoadingLayout创建不同的LoadingLayout
